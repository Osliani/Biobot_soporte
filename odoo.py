...
def get_helpdesk_ticket():
    ...
    domain = []
    fields = ["name", "description", "priority", "stage_id", "user_id"]
    token = get_oauth_token()
    limit = 1
...
def get_helpdesk_ticket():
    ...
    product_data = fetch_odoo_data(model, domain, fields, token, limit)
    
    return product_data


def create_helpdesk_ticket(name, description, priority, stage_id, user_id, email_cc=None, closed_date=None):
    token = get_oauth_token()
    ...
    if email_cc:
        data['args'][0][0]['email_cc'] = email_cc
        
    if closed_date:
        data['args']['closed_date'] = closed_date
...
if __name__ == "__main__":
    new_ticket = create_helpdesk_ticket(
        name = "Test3",
