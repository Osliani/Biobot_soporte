...
def repuestos(tipo_producto, modelo):
    return "Responde lo siguiente: Herramienta repuestos en desarrollo..."


def compatibilidades(tipo_producto, producto):
    return "Responde lo siguiente: Herramienta compatibilidades en desarrollo..."


def software(tipo_producto, producto):
    return "Responde lo siguiente: Herramienta software en desarrollo..."


def stock(tipo_producto, producto):
    return "Responde lo siguiente: Herramienta stock en desarrollo..."
...
def user_actions():
    ans = "Estas son las acciones que puedes realizar: \n" \
        ...
        "- Consultar los puntos de ventas de Marsgaming en España. \n"\
        "- Responder inquietudes a los clientes. \n"\
        "- Ofrecer información sobre la empresa. \n"\
        "- Registrar quejas y sugerencias para ser atendidas por nuestro equipo."
    
    return ans


def create_ticket(name, description, priority, email_cc=None, closed_date=None):
    try:
        ticket = odoo.create_helpdesk_ticket(name, description, priority, 1, 1, email_cc, closed_date)
        print(ticket)
        
        return "El ticket se ha creado!"
    
    except Exception as exc:
        print(exc)
        
        return "Error creando el ticket."
